...
filename <- 'data/NosAutoanalizamos_TD2019 - Hoja 1 - Clean.tsv'
Datos <- read_tsv(filename)
Datos <- Datos %>% mutate(across(ends_with(".cat"),as.factor))
```
...
View(Datos)
```
Hay más de una variable categórica de tipo string sin normalizar
...
Datos %>% 
  summarise(across(ends_with(".num"),list(media=~mean(.x,na.rm=TRUE),sd=~sd(.x,na.rm=TRUE),mediana=~median(.x,na.rm=TRUE), iqr = ~IQR(.x,na.rm=TRUE)))) %>% 
  pivot_longer(everything(), names_to = "nombres_combinados", values_to = "valores") %>% 
  separate(nombres_combinados,c("asignatura","stat"),"_") %>% 
  pivot_wider(names_from = stat, values_from = valores) %>% 
  mutate(mean_sd = paste(media, sd, sep = "±"))
...
ggplot(Datos, aes(x = ALG.num)) + geom_histogram(aes(y=stat(density))) + geom_density() + geom_vline(xintercept = mean(Datos$ALG.num,na.rm=TRUE))
```
...
valores <- Datos %>% pivot_longer(cols=ends_with(".num"), names_to = "asignatura", values_to = "valor") %>% 
...
mosaicplot(~ ALG.cat + ANM.cat, data = Datos)
```
...
Datos <- Datos %>%  mutate(sqrtALG.num = sqrt(ALG.num))
cat("Pearson\n")
Datos %>% 
  select(ends_with(".num"),) %>% 
  cor(use = "complete.obs", method = "pearson")
...
attach(Datos)
chisq.test(ALG.cat,ANM.cat,correct=FALSE)
```
Como $p<0,05$, rechazamos la hipótesis nula: debe existir dependencia entre los tipos de calificaciones de ALG y ANM.

```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
chisq.test(ALG.cat,Sex,correct=FALSE)
```
Como $p>>0,05$, asumimos la hipótesis nula: no debe existir dependencia entre los tipos de calificaciones de ALG y la variable Sex.
